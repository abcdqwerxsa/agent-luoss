...
- [ ] **S1 脚手架**：buf + proto 六个服务定义与代码生成；单 go.mod 多 cmd；Makefile（build/gen/up）；compose 起 postgres+redis；PG 各 schema 迁移文件
- [ ] **S2 iam + gateway 骨架**：用户表/角色/JWT（access+refresh）；登录、用户 CRUD；gateway JWT 中间件与路由透传用户声明；审计拦截器（生产 `stream:audit`）
- [ ] **S3 agent-runtime**：gRPC server；pi 集成（CreateSession 带模式/工具白名单/ResourceLoader）；Prompt/Steer/Abort/PushEvents；会话池 + 空闲驱逐 + 文件恢复；Heartbeat；本地用官方 API Key 冒烟（models.json 手写）
