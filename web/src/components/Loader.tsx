...
/* Ported from Beautiful UI "Loading State" (MIT License, © 2026 Shane Levine,
 * https://www.beautifului.dev/) — pixel-grid loader with shimmering label and
 * a live elapsed timer. Surfer/video variant dropped. */
...
export default function Loader({ label = "处理中", variant = "Drive" }: { label?: string; variant?: "Drive" | "Dots" | "Orbit" }) {
  const elapsed = useElapsed();
  const { delays, dur, round } = PATTERNS[variant] ?? PATTERNS.Drive;
  return (
    <div role="status" className="bui flex w-fit items-center gap-2.5">
